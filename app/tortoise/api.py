...
def format_conditioning(clip, cond_length=132300):
    ...
    return mel_clip.unsqueeze(0).cpu()
...
def classify_audio_clip(clip, models_folder_path: str):
    """
    ...
    classifier.load_state_dict(
        torch.load(
            f"{models_folder_path}/classifier.pth", map_location=torch.device("cpu")
        )
    )
...
class TextToSpeech:
    ...
    def __init__(
        ...
        if os.path.exists(f"{models_dir}/autoregressive.ptt"):
            ...
            self.diffusion = torch.jit.load(f"{models_dir}/diffusion_decoder.pth")
        else:
            self.autoregressive = (
                UnifiedVoice(
                    max_mel_tokens=604,
            ...
            self.autoregressive.load_state_dict(
                torch.load(
                    f"{models_dir}/autoregressive.pth", map_location=torch.device("cpu")
                )
            )

            # ADDITION FMT
            # torch.save(self.autoregressive, f"{models_dir}/autoregressive.ptt")
            # print(f"Now, self.autoregressive is saved to {models_dir}/autoregressive.ptt")
            # ADDITION FMT
            ...
            self.diffusion.load_state_dict(
                torch.load(
                    f"{models_dir}/diffusion_decoder.pth",
        ...
        self.clvp.load_state_dict(
            torch.load(f"{models_dir}/clvp2.pth", map_location=torch.device("cpu"))
        )
        ...
        self.cvvp.load_state_dict(
            torch.load(f"{models_dir}/cvvp.pth", map_location=torch.device("cpu"))
        )

        self.vocoder = UnivNetGenerator().cpu()
        self.vocoder.load_state_dict(
            torch.load(f"{models_dir}/vocoder.pth", map_location=torch.device("cpu"))[
                "model_g"
            ]
        )
    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        """
        with torch.no_grad():
            voice_samples = [v.to("cpu") for v in voice_samples]
            ...
            for sample in voice_samples:
                ...
                cond_mel = wav_to_univnet_mel(sample.to("cpu"), do_normalization=False)
    ...
    def get_random_conditioning_latents(self):
        ...
        if self.rlg_auto is None:
            ...
            self.rlg_auto.load_state_dict(
                torch.load(
                    f"{self.models_dir}/rlg_auto.pth",
                    map_location=torch.device("cpu"),
                )
            )
            ...
            self.rlg_diffusion.load_state_dict(
                torch.load(
                    f"{self.models_dir}/rlg_diffuser.pth",
                    map_location=torch.device("cpu"),
                )
            )
        with torch.no_grad():
            return (
                self.rlg_auto(torch.tensor([0.0])),
                self.rlg_diffusion(torch.tensor([0.0])),
            )
    ...
    def tts(
        ...
        **hf_generate_kwargs,
        ...
        """
        text_tokens = torch.IntTensor(self.tokenizer.encode(text)).unsqueeze(0).cpu()
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(samples, disable=not verbose):
                ...
                if auto_conds is not None:
                    ...
                    clip_results.append(
                        clvp * clvp_cvvp_slider + cvvp * (1 - clvp_cvvp_slider)
                    )
                else:
